...
# Starting entrypoint index: 0 is set_key, 1 is handle_input, 2 is handle_handler
START_ENTRY_INDEX   = 3
...
# Regex
# Note: [ \t] means space or tab. I use this when I don't explicitly allow newlines (e.g., in comments)
#       \s means any space char (also newlines). This is for non-comment lines
REGEX_LAZY = "^\s*#\s*[\s*macro_use\s*]\s*extern\s*crate\s*lazy_static\s*;"
REGEX_OUTPUT = "^[ \t]*//@[ \t]*sm_output[ \t]*\([ \t]*(?P<fname>_?[a-zA-Z]+[_a-zA-Z0-9]*)[ \t]*\)[ \t]*$"
REGEX_INPUT = "^[ \t]*//@[ \t]*sm_input[ \t]*\n\s*pub\s+fn\s+(?P<fname>_?[a-zA-Z]+[_a-zA-Z0-9]*)\s*\(\s*_?[a-zA-Z]+[_a-zA-Z0-9]*\s*:\s*&\s*\[\s*u8\s*]\s*\)\s*\{"
REGEX_ENTRY = "^[ \t]*//@[ \t]*sm_entry[ \t]*\n\s*pub\s+fn\s+(?P<fname>_?[a-zA-Z]+[_a-zA-Z0-9]*)\s*\(\s*_?[a-zA-Z]+[_a-zA-Z0-9]*\s*:\s*&\s*\[\s*u8\s*]\s*\)\s*->\s*ResultMessage\s*\{"
REGEX_REQUEST = "^[ \t]*//@[ \t]*sm_request[ \t]*\([ \t]*(?P<fname>_?[a-zA-Z]+[_a-zA-Z0-9]*)[ \t]*\)[ \t]*$"
REGEX_HANDLER = "^[ \t]*//@[ \t]*sm_handler[ \t]*\n\s*pub\s+fn\s+(?P<fname>_?[a-zA-Z]+[_a-zA-Z0-9]*)\s*\(\s*_?[a-zA-Z]+[_a-zA-Z0-9]*\s*:\s*&\s*\[\s*u8\s*]\s*\)\s*->\s*Vec\s*<\s*u8\s*>\s*\{"
